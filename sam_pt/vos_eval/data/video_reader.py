...
class VideoReader(Dataset):
    ...
    def __init__(self, vid_name, image_dir, mask_dir,
        ...
        if shortest_size < 0 and longest_size is None:
            self.resize_longest_side_transform = None
            self.im_transform = transforms.Compose([
                transforms.ToTensor(),
            ])
        elif shortest_size > 0:
            self.resize_longest_side_transform = None
            self.im_transform = transforms.Compose([
                transforms.ToTensor(),
                transforms.Resize(shortest_size, interpolation=InterpolationMode.BILINEAR),
            ])
        elif longest_size is not None:
            self.resize_longest_side_transform = ResizeLongestSide(longest_size)
            self.im_transform = transforms.Compose([
                transforms.ToTensor(),
            ])
        else:
